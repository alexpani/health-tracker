...
- `/journal` — **Diario**. Pagina dedicata con sidebar filtri (casella di ricerca testuale debounced su `content_text`, chip tag dinamici da `useJournalTags`, range date con preset 7g/30g/90g/1a/Tutto). Lista raggruppata cronologicamente per mese (mese piu' recente in alto); ogni voce mostra giorno+data, chip tag, preview troncata del plain text. Click sulla voce → `/day/<date>` (la modifica avviene li' tramite la card Diario). Filtri persistiti in `sessionStorage` chiave `journal_filters_v1`. Backed by `GET /api/v1/journal`.
