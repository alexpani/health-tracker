...
- `/vitals` — HR, HRV, SpO2, blood pressure, respiratory, temperature, glucose. In cima alla pagina **card "FC a riposo vs periodi di allenamento"** (`components/RestingHRTrainingOverlay.tsx`): linea della RHR media mensile (`HKQuantityTypeIdentifierRestingHeartRate` aggregato `monthly`) con le fasce `regimens kind='training'` (autodetect) sovrapposte come `ReferenceArea` ambra. Header con media RHR mesi in-allenamento vs OFF + Δ colorato (verde se calo). Tooltip mostra avg/min/max del mese e l'elenco delle fasce attive in quel mese.
