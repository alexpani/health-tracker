...
- `JournalEntry` — voce diario giornaliera (free-form, rich text + tag). Colonne: `date` (UNIQUE, una sola entry per giorno), `content_html` (HTML sanitizzato server-side via `bleach`, whitelist tag testuali + heading + liste + link), `content_text` (plain text estratto per ricerca ILIKE), `tags` JSONB (array di stringhe normalizzate lowercase trim dedup), `created_at`, `updated_at`. Indice GIN su `tags`. Visualizzata come card "Diario" su `/day/:date` in cima alla griglia, e listabile dalla pagina dedicata `/journal` con ricerca testo e filtro per tag. Niente HK / iOS — feature solo dashboard.
...
- `journal.py` — CRUD `/api/v1/journal` per voce diario giornaliera (rich text + tag). `POST` e' upsert su `date` (`ON CONFLICT DO UPDATE`): una sola entry per giorno. `content_html` passa per `bleach.clean` con whitelist (`p, br, strong/b, em/i, u, s, h1-h4, ul, ol, li, blockquote, code, pre, a`) prima di entrare in DB; `content_text` derivato e' salvato in colonna omonima per ricerca. Tag normalizzati lowercase trim dedup max-50. **Ricerca**: `text_contains` con singola parola usa ILIKE `%x%`; con piu' parole/quote usa `search_tsv @@ websearch_to_tsquery('italian', q)` (stemming italiano, AND multi-parola, supporto `"phrase"` e `-negation`). Endpoint extra: `GET /by-date/{YYYY-MM-DD}` (entry singola o 404), `GET /days?start=&end=` (pallini mini-calendario), `GET /tags` (autocomplete, distinct tag ordinati per frequenza desc via `jsonb_array_elements_text`), `POST /bulk` body `{ids, action: "delete"|"add_tag"|"remove_tag", tag?}` (azione in massa, ritorna `{updated}` o `{deleted}`), `POST /tags/rename` body `{old, new}` (rinomina su tutte le voci; `new: null` rimuove il tag ovunque).
