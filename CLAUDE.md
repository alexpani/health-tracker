...
- `/vitals` — HR, HRV, SpO2, blood pressure, respiratory, temperature, glucose. Nel tab **Battito a riposo** (in coda a grafico + tabella) viene mostrata la **card "FC a riposo vs periodi di allenamento"** (`components/RestingHRTrainingOverlay.tsx`): linea della RHR media mensile con le fasce `regimens kind='training'` (autodetect) sovrapposte come `ReferenceArea` ambra. Nel tab **HRV** in coda viene mostrata la **card "Recupero stimato"** (`components/RecoveryCard.tsx` + logica pura in `lib/recoveryScore.ts`): score 0-100 (label Pronto/Buono/Cauto/Scarso) calcolato come media pesata di HRV (peso 0.5), FC a riposo (0.25) e Sleep score (0.25). HRV e RHR sono z-score sul baseline rolling 30g personale, mappati linearmente in [0,1] con saturazione a ±2σ. Sleep score riusa `lib/sleepScore.computeSleepScore`. Se uno dei 3 segnali manca i pesi vengono rinormalizzati (badge "parziale"); se mancano tutti la card mostra "dati insufficienti". Tutto frontend-only, niente endpoint backend dedicato — la card fa 3 query (`useSamples` HRV/RHR + `useCategories` sleep) sugli ultimi 31 giorni. Le card vitali sono agganciate via la prop `extrasByType` di `TypeBrowser` (mappa `typeIdentifier → ReactNode`).
