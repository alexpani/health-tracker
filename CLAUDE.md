...
Tutto frontend-only, niente endpoint backend. Le date sono `useMemo`-stabilizzate per giorno locale (chiave `new Date().toDateString()`) per evitare loop di refetch di TanStack Query. Le card vitali sono agganciate via la prop `extrasByType` di `TypeBrowser` (mappa `typeIdentifier → ReactNode`).
