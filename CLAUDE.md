...
- **MCP server** (`mcp/`): Python 3.13 + FastMCP (SDK ufficiale Anthropic) + asyncpg + httpx, deploy via systemd nativo (no Docker) su LXC `ealth-mcp`. Espone Health Tracker a Claude (Desktop, Code, claude.ai connector) come MCP read-only. **13 tool**: `query_sql`/`describe_schema`/`describe_table` (SQL libero su utente PG `health_ro` con SELECT-only + statement_timeout 10s + auto-LIMIT 5000); `get_day`/`get_active_regimens`/`get_health_profile` (sintesi via FastAPI backend); `aggregate`/`compare_periods`/`correlate`/`find_periods`/`life_timeline` (primitive analitiche su bucket day/week/month/quarter/year con avg/sum/min/max/median/count/stddev/slope_per_day; correlate supporta Pearson e Spearman); `list_metrics`/`reload_metrics_catalog`. **3 resource statiche** caricate automaticamente dal client: `profile://me` (anagrafica + baselines correnti), `metrics://catalog` (lista slug aggregabili per categoria), `glossary://project` (convenzioni semantiche: codici HKWorkoutActivityType, effective_type slug, gotcha body_fat 0-1, total_distance in metri ecc.). **Catalogo metriche estensibile** in `mcp/metrics.yaml` (~45 slug iniziali: body, vitals, activity, workout, nutrition, sleep, lab): ogni metrica e' una subquery che ritorna `(t, v)`, hot-reload via tool. **Auth**: il token segreto e' nel PATH dell'URL (`/mcp/<64-hex>`) perche' claude.ai web non supporta header custom nei connector — entropia equivalente a un bearer header. **NPM** (`healthmcp.activeproxy.it`) fa TLS termination + reverse-proxy a `192.168.68.100:8765`. **HostHeaderRewrite middleware** in `server.py` riscrive il Host header a `localhost:8765` prima che MCP applichi il proprio check anti DNS-rebinding (che rifiuterebbe l'hostname pubblico; il rischio non si applica perche' il path-secret e il reverse-proxy fidato bastano). Niente scritture sul DB: solo SELECT, niente write back verso Apple Health.
