...
- `Info.plist`: `NSHealthShareUsageDescription`, `NSHealthUpdateUsageDescription`, `NSLocationAlwaysAndWhenInUseUsageDescription`, `NSLocationWhenInUseUsageDescription`, `BGTaskSchedulerPermittedIdentifiers=["com.healthtracker.sync"]`, `UIBackgroundModes=["fetch","processing","location"]`
- Bundle identifier: `com.alexpani.healthtracker.app` (paid Apple Developer Program — sblocca Push Notifications + Clinical Health Records).
- Wake-up channels attivi: HKObserverQuery (BG delivery), BGAppRefreshTask (subject to iOS scheduling policy), **Significant Location Changes** (CoreLocation), `protectedDataDidBecomeAvailable` (con warm-up 2s), e — pianificato post-passaggio al paid program — **silent push APNs** come trigger remoto on-demand (vedi piano `silent push + clinical records` per i dettagli implementativi).
