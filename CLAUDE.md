...
- `/` — **Home**: today metric cards, weekly charts, last 3 workouts, sync status card, **last 10 sync sessions table** (con badge "wake-up" + opacita' ridotta sulle righe `total_samples=0` per distinguere i sync "produttivi" da quelli generati da wake-up in background che non hanno trovato dati nuovi — es. SLC fired con device lockato; testo esplicativo in cima alla card), **bottone "Sincronizza"** in alto a destra (`components/SyncButton.tsx`) che fa `refetchQueries` su tutte le 16 query key dipendenti dai dati sync iOS (`samples`, `latest`, `latestWeight`, `sampleFacets`, `categories`, `types`, `dailyStats`, `daySnapshot`, `workouts`, `workout`, `workoutSplits`, `workoutFacets`, `workoutRecords`, `workoutRecordsFacets`, `syncStatus`, `syncSessions`). Spinner + label "Aggiornando..." durante il refetch.
