...
- Personal teams cannot use Push Notifications either — silent push wake-ups are blocked until a paid Apple Developer Program account is active. Until then the wake-up channels are: HKObserverQuery (BG delivery), BGAppRefreshTask (subject to iOS scheduling policy), and **Significant Location Changes** (CoreLocation, see below).
...
**Note sui sync "wake-up" vuoti**: quando il telefono e' lockato in BG e l'app si sveglia (qualunque trigger), HealthKit ritorna 0 sample per i tipi protetti (StepCount, HeartRate, AEE, BEE, ...) senza throwing. Il fix di `isAppInBackground()` in `SyncService` impedisce di avanzare anchor / `lastSyncDate` su empty fetch in BG, cosi' nessun dato viene perso — i sample arrivano al primo sync con device sbloccato. Questi sync vuoti vengono comunque registrati lato backend via `/sync/heartbeat` e segnalati nella dashboard Home (badge "wake-up" sulle righe con `total_samples=0` nella tabella "Ultime sincronizzazioni").
