...
- `/vitals` — HR, HRV, SpO2, blood pressure, respiratory, temperature, glucose. Nel tab **Battito a riposo** (in coda a grafico + tabella) viene mostrata la **card "FC a riposo vs periodi di allenamento"** (`components/RestingHRTrainingOverlay.tsx`): linea della RHR media mensile con le fasce `regimens kind='training'` (autodetect) sovrapposte come `ReferenceArea` ambra. Nel tab **HRV** in coda vengono mostrate due card affiancate: **"Recupero stimato"** (`components/RecoveryCard.tsx` + logica pura in `lib/recoveryScore.ts`) e **"Pronto per domani?"** (`components/ReadinessCard.tsx` + `lib/readinessScore.ts`). RecoveryCard: score 0-100 (Pronto/Buono/Cauto/Scarso) come media pesata di 5 biomarker NOTTURNI — HRV (0.35), FC a riposo (0.25), Frequenza respiratoria (0.15), Saturazione O2 (0.10), Sleep score (0.15). HRV/RHR/RR/SpO2 sono filtrati alla finestra di sonno [yesterday 22:00, today 10:00 locale] tramite `nightlyAverage()` per evitare contaminazione da letture diurne (es. Breathe HRV) come fa Bevel; poi z-score sul baseline rolling 30g personale, saturazione a ±2σ; RHR/RR sono `lowerIsBetter: true` (segno invertito). Sleep score riusa `lib/sleepScore.computeSleepScore`. ReadinessCard ("Stima recupero domani"): stima lo stato di domani mattina combinando stato attuale (0.35, dal recupero stamattina), ACWR Acute:Chronic Workload Ratio (0.3, kcal totali 7g acuto / media settimanale 28g cronico, sweet spot 0.8-1.3 secondo Gabbett 2016/Hulin 2014), **sonno previsto stanotte** (0.2, mediana ultime 7 notti vs baseline 30g, z-score con saturazione a ±2σ — assume pattern di sonno stabile) e trend HRV ultimi 7g (0.15, pendenza lineare); lista di bullet `ok/warn/bad` con motivazioni + breakdown numerico Carico 7g/Media 28g/ACWR/Sonno stimato. Se un segnale manca i pesi vengono rinormalizzati (badge "parziale"). Tutto frontend-only, niente endpoint backend — le card fanno 4 query (`useSamples` HRV/RHR + `useCategories` sleep + `useWorkouts` ultimi 35g). Le date sono `useMemo`-stabilizzate per giorno locale (chiave `new Date().toDateString()`) per evitare loop di refetch di TanStack Query. Le card vitali sono agganciate via la prop `extrasByType` di `TypeBrowser` (mappa `typeIdentifier → ReactNode`).
