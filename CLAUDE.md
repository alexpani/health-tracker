...
- `GET /api/v1/workouts/records` — personal records per effective_type honoring the same filters as `/workouts`. Returns for each activity: `overall` (longest_distance, longest_duration, fastest_pace, most_calories), `at_distance` (best time at 5/10/21.097/42.195 km, within +10% tolerance), `best_single_km` for running/walking (fastest 1-km split reconstructed from distance samples, sampled from top-30 candidates by average pace, with a 3:00/km floor to reject GPS artifacts)
...
- `/workouts` — **main Workouts page** with **left sidebar filters** (year, activity, source, datetime, distance km, duration min, pace slider + presets, title search, notes search). Two tabs:
  - **Elenco**: summary cards, workouts-per-period chart with click-to-drilldown, **sortable** list table (click headers to sort asc/desc) with **title**, pace and truncated notes columns, row-level delete with 8s undo toast.
  - **Record**: per-activity Personal Records card (distanza max, durata max, pace medio migliore, kcal max, best time at 5/10/21/42 km when available, miglior km ever for running/walking). Honors all sidebar filters. Each record clicks into the workout detail. Backed by `/api/v1/workouts/records`.
