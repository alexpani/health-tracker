...
- **Workouts**: rich filtering by `effective_types` (canonical slugs derived from `activity_type` + metadata, e.g., `treadmill_run`, `swim_pool`), `years[]`, `sources[]`, distance/duration/pace ranges. Single workout detail + per-km splits + delete (with snapshot for undo) + notes PATCH. **Personal records** endpoint (`/workouts/records`) with overall/per-distance/best-single-km, filter-aware.
...
- **Workout** — main page with **left sidebar filters** (year, activity chips, source, datetime range, distance km, duration min, **pace dual-range slider + preset chips**, title search, notes search). Two tabs: **Elenco** (stats, chart with click-to-drilldown zoom year→month→week→day→workout, sortable list, row delete with 8s undo — third-party titles like Intervals Pro "Corsa Livello 2" promoted to a dedicated column) and **Record** (personal records per activity: distanza/durata/pace/kcal max, miglior tempo a 5/10/21/42 km, miglior km ever per corsa/camminata). Filters **persisted in sessionStorage** so they survive navigation.
- **Workout detail** (`/workouts/:uuid`) — Apple Fitness-style: metrics (duration, distance, calories, pace, HR), "Informazioni aggiuntive" card (indoor/outdoor, swim location, lap length, elevation, METs, weather, brand), **per-km splits** table, **Intervalli** card with per-lap distance/pace/HR/kcal (Intervals Pro, Apple Watch strutturati, Strava, HealthFit, ecc.), time-series charts (HR, running speed, power, cadence), **editable title and notes** cards
